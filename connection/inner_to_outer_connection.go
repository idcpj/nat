...
import (
	"encoding/json"
	"errors"
	"github.com/idcpj/go_nat/internal"
	"io"
	"log"
	"net"
	"strconv"
...
func (itoc *InnerToOuterConnection) communicate(single int) error {
	...
	case internal.Ping:
		//ping
		sb = internal.GenerateSignal(internal.Ping, "")
	case internal.Auth:
		//授权
		sb = internal.GenerateSignal(internal.Auth, itoc.OutServerAuthKey)
	case internal.StartProxy:
		//准备转发
		sb = internal.GenerateSignal(internal.StartProxy, "")
	default:
...
func (itoc *InnerToOuterConnection) Read() {
	for {
		...
		buf := make([]byte, internal.PackageLength)
...
func (itoc *InnerToOuterConnection) Register() {
	...
	err = itoc.communicate(internal.Auth)
	if err != nil {
		//注册失败
...
func (itoc *InnerToOuterConnection) Ping() {
	...
	err := itoc.communicate(internal.Ping)
	if err != nil {
...
func (itoc *InnerToOuterConnection) Proxy() {
	...
	itoc.StatusMonitor(itoc.Id, itoc.Status)
	//发送转发信号
	err := itoc.communicate(internal.StartProxy)
	if err != nil {
